...
function updateStudentEmail() {
    ...
    if (selectedDomain === 'custom') {
        // 직접 입력 선택 시
        studentEmailInput.value = emailId + '@' + domainCustom;
    } else {
        // 다른 옵션 선택 시
        studentEmailInput.value = emailId + '@' + selectedDomain;
    }
}


// 값이 변경될 때마다 updateemployeeEmail 함수 호출
emailIdInput.addEventListener('input', updateStudentEmail);
domainCustomInput.addEventListener('input', updateStudentEmail);
domainListSelect.addEventListener('change', updateStudentEmail);
